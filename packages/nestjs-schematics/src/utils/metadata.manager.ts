...
export class MetadataManager {
  ...
  public insert (
    ...
    const addBlankLinesIfDynamic = (): void => {
      symbol = (staticOptions != null) ? this.addBlankLines(symbol) : symbol
    }
  ...
  private insertSymbolToMetadata (
    ...
    if (arrLiteral.elements === undefined) {
    ...
    if ((node as Node).kind === SyntaxKind.ArrayLiteralExpression) {
      position--
      toInsert = (staticOptions != null) ? this.addBlankLines(symbol) : `${symbol}`
    } else {
      const text = (node as Node).getFullText(source)
      if (text.match(/^\r?\n/) != null) {
        const match = text.match(/^\r?\n(\r?)\s+/)?.shift() ?? ''
        toInsert = `,${match}${symbol}`
      } else {
        toInsert = `, ${symbol}`
      }
    }
